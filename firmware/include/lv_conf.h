/**
 * @file lv_conf.h
 * Philly Transit Display firmware config for LVGL 9.x, built from the
 * lv_conf_template.h shipped with lvgl/lvgl v9.2.2 (the version
 * rzeldent/esp32_smartdisplay@2.1.1 declares as its dependency), cross-
 * checked against the working config in rzeldent/esp32-smartdisplay-demo
 * (both fetched 2026-09-13), with only the font-size selection changed for
 * this project's UI (DESIGN.md SS8). firmware/platformio.ini's `^9.2.2`
 * range actually resolves to lvgl 9.5.0 (the newest 9.x on the registry) -
 * this template-based config built and ran fine against it (verified via
 * `pio run`), consistent with LVGL's within-major-version config stability.
 * Referenced via the absolute path build flag
 * -D LV_CONF_PATH="${platformio.include_dir}/lv_conf.h" (see
 * platformio.ini - note the embedded quotes, without which `#include
 * LV_CONF_PATH` fails to compile; see platformio.ini's comment on this);
 * PlatformIO caches libraries after first build, so delete firmware/.pio
 * if you edit this file and the change doesn't seem to take effect.
 *
 * Every LV_USE_<widget> the template turns on by default (chart, calendar,
 * keyboard, msgbox, table, tabview, tileview, win, qrcode, grid, observer,
 * and more) is disabled here except LV_USE_LABEL - this project's three
 * screens (main_screen.cpp/device_info_screen.cpp/stats_screen.cpp) only
 * ever create lv_obj containers, lv_label text, and flex layouts. Verified
 * by actually building: with everything the template enables by default,
 * cyd-3248S035R's firmware.bin was 1,875,882 bytes against a 1,310,720-byte
 * app partition (143%, `pio run`'s checkprogsize step fails outright);
 * trimmed to just what's used, it fits (see this project's build report
 * for the final size). Re-enable a widget here if UI code starts using it.
 */
...
#define LV_USE_STDLIB_SPRINTF   LV_STDLIB_BUILTIN
    ...
    /* Overridable only so the host screenshot simulator (firmware/sim, 64-bit pointers) can run
     * the same screens in a bigger pool; no board build defines it. */
    #ifndef LV_MEM_SIZE
...
#define LV_USE_IMAGE      1   /*Requires: lv_label; the header weather icons (src/icons)*/
