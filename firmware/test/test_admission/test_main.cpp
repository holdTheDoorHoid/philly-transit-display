...
void test_either_heap_floor_refuses_on_its_own() {
  TEST_ASSERT_FALSE(admitConnection(0, kAcceptMinFree8 - 1, 24 * 1024));
  TEST_ASSERT_FALSE(admitConnection(0, 40 * 1024, kAcceptMinLargestBlock - 1));
  // Exactly at both floors is admitted: they are minimums, not exclusive bounds.
  TEST_ASSERT_TRUE(admitConnection(0, kAcceptMinFree8, kAcceptMinLargestBlock));
}
...
void test_the_state_that_crashed_rc2_is_refused() {
  ...
  // And once the heap really is gone, nothing is admitted at any count.
  TEST_ASSERT_FALSE(admitConnection(0, 2000, 1500));
}
